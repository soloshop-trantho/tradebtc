...
def detect_tweezer(candles, idx, upper_band, lower_band):
    """Trả về 'tweezer_top' hoặc 'tweezer_bottom' nếu đủ điều kiện hình dáng + chạm dải Bollinger."""
    if idx < 1:
        return None
    cur = candles[idx]
    prev = candles[idx - 1]
    ...
    # --- Đáy Nhíp (Tweezer Bottom): đỏ rồi xanh, đáy gần bằng nhau, chạm dải dưới ---
    ...
    # --- Đỉnh Nhíp (Tweezer Top): xanh rồi đỏ, đỉnh gần bằng nhau, chạm dải trên ---
...
def main():
    ...
    for interval in INTERVALS:
        ...
        # ---------- 1) Xử lý xác nhận Hammer đang chờ (nếu có) ----------
        ...
        if tweezer:
            ...
            if key not in alerted:
                alerted.add(key)
                changed = True
                msg = build_tweezer_message(SYMBOL, interval, tweezer, candles, idx, upper_band, lower_band)
                send_telegram(msg)
